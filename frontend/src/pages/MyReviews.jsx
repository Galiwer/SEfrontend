...
export default function MyReviews() {
  ...
  return (
    <div className="reviews-page">
      ...
      {loading ? <div className="rv-state">Loading your reviews...</div>
      : error ? <div className="rv-state">{error}</div>
      : reviews.length === 0 ? <div className="rv-state">You haven’t posted any reviews yet.</div>
      : (
        <div className="reviews-container">
          <div className="reviews-grid">
            {reviews.map((r) => (
              <article key={r.id} className="review-card" aria-label={`Your review from ${new Date(r.createdAt).toLocaleDateString()}`}>
                <header className="review-header">
                  <div className="review-avatar" aria-hidden="true" />
                  <div className="review-name">You</div>
                  {r.createdAt && (
